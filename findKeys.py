...
def neighContours( txt_, pts_, json_, conj_lt=None, conj_rt=None ):
    ## 3 neigh in either dirn for every contour
...
def processNeighbours( json_, json_raw, fileNm ):
    '''
    input args -> json o/p ( stitched ..basically take in the raw json o/p and add 2 contours together
                             if the distance between them is too low ..a tad heuristic but hey, thats life )
                  json raw ( this is the input for the stitching algorithm )
                  file name ( just the name of the file ! )
     ** if you do NOT have an algorithm to stitch and just have a json ( we assume a google ocr o/p format as shared
        in the sample json folder ) then you could use the same json as arg # 1 and 2 ** 
     
    under the hood -> the idea is to  
      - go contour by contour and find contours of interest by relying on a simple rule. The neighbour of the 
        current contour ( to its RT or BELOW ) needs to be of a different DATATYPE. So if we find a contour
        "Invoice NO" and to its right , is an alphanumeric value "A12312" then this pair becomes a training
        input. Please note that since we wanted to use a completely UNSUPERVISED method of training ( sans
        annotations ) we have to rely on smart rules like this. The only case that this will not be able to 
        identify would be cases where BOTH the KEY and VALUE had the same data type. ( "Name:" -> "Murphy" )
        but thats why we also use a simple feature vector to represent its entire neighbourhood. So we use
        a simple 3 contours above , below, left and right ( hence the neighbourhood is a 6 x feats for vertical
        and a 6 x feats for horizontal vectors )    

        so the idea is to mark such contours with labels. Current schema is KEY-VALUE-RIGHT, KEY-VALUE-BELOW,
        VALUE-KEY-LEFT, VALUE-KEY-TOP and IRR-NA ..the advantage of labelling both potential KEYS and VALUES
        is a factor of redundancy thats added since if the model misses predicting a KEY, the VALUE will certainly
        be predicted. 

        during inference we simply take ALL the contours and pass them through the same algorithm and based
        on the predictions, we extract all the key value pairs from any category of documents
    ...
    with open( 'DATA_ENHANCED_KV/'+fileNm, 'wb' ) as fp:
      np.save( fp, resInp_ ) 
...
if __name__ == '__main__':
    ...
    the below folders need to point to the OCR output of your system ..like mentioned above 
    IF you DO NOT have an algorithm to stitch the OCR o/p, you can use the same folder path
